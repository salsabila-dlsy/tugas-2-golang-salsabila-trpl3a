...
func getNilai(w http.ResponseWriter, r *http.Request) {
	...
	for result.Next() {
		...
		for resultNilai.Next() {
			err := resultNilai.Scan(&nilain.NoBp, &nilain.IDMatkul, &nilain.NamaMatkul, &nilain.Nilai, &nilain.Semester)
			if err != nil {
				panic(err.Error())
			}
			mhs.Nilai = append(mhs.Nilai, nilain)
		}
	}
